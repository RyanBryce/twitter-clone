$(document).ready(function() {
  ...
  $('#area').keyup(function() {
    var chars = $(this).val().length; //gets length of texare
  ...
  $('#tweet-submit').on('click', function() {
      $('#tweet-controls').addClass('tweet-controls-hidden');        $('.tweet-compose').removeClass('tweet-compose-dubH');
      var message = $('#area').val();
        $('#stream').prepend(
            ...
            '<div class="stats">' +
              ...
              '</div>' +
              '<div class="favorites">' +
                '<p class="num-favorites">'+initVal+'</p>' +
                '<p>FAVORITES</p>' +
              '</div>' +
      ...
      var initVal = 0;
      $('.num-favorites').text(initVal);
      $('.action-favorite').click(function() {
       var numFavoritesElem = $(this)
                         .closest('.content')
                         .find('.num-favorites');

      var numFavorites = Number(numFavoritesElem.text());
      numFavoritesElem.text(numFavorites + 1);
     });
     localStorage.setItem('stream', 'Crazy Ted');
      $('#stream').prepend(localStorage.getItem('stream'))
